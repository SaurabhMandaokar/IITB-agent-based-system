import time
import pandas as pd
from BaseLibraries.support_files import map_performative_type, pointing, function_execution, create_a_reply_to_send, \
    map_and_point
...
def handle_fipa_contract_net_protocol(message, msg_to_send_queue, paths_dictionary,allotment_queue):
    ...
    if performative in ['cfp', 'call for proposal', 'call-for-proposal', 'call_for_proposal']:
        ...
        if executable_function_name is not None:
            ...
            result_dict = function_execution(executable_function_name, message.type, message.content)
            print('function execution results:', result_dict)
            if result_dict is None:
                reply_parameters = {}
                reply_parameters["reply_performative"] = 'propose'
                reply_parameters["reply_content"] = random.randint(1, 10)
                reply_parameters["reply_type"] = 'propose_cost'
        ...
        allotted_name = message.sender
        allotment_queue.put(allotted_name)
